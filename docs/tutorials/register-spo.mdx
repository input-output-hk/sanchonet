...
role in propagating the blocks forged by your Block Producer node, as well as fetching blocks from other stake pools and serving them to your Block Producer. In this structure, 
...
The typical configuration of your stake pool should resemble the following:

![diagram](@site/static/img/bpandrelay.png)


To set up your Relay node, you will need to carry out the following steps:

1. **Install the Cardano-node and Cardano-cli** on your Relay node. 
...
and port of your Block Producer node. This step ensures that your Relay node will always maintain a connection with your Block Producer node.
...
### Setup a block producer 

Setting up your Block Producer involves similar steps, but there are a few differences on the configuration: 

1. **Install the Cardano-node and Cardano-cli** on your Block Producer node. 
...
and port of your **own Relay node**. Note that the Block Producer does not connect to `sanchonet-node.world.dev.cardano.org`and we avoid using ledger peers by setting `"useLedgerAfterSlot¨`to `-1`
