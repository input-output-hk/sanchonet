...
### Common aspects to all types of governance actions:
...
When using the cardano-cli to create a governance action you will notice that it is really creating a proposal. 
...
```
cardano-cli conway governance query gov-state --testnet-magic 42 | jq .ratify.prevGovActionIds
...
The document at the URL can be of a free form. The important aspect of it is that it should communicate to ada holders the what and the why of the proposal. For the most 
this tutorial uses https://tinyurl.com/3wrwb2as as an example. 
...
Assume that the individual or entity that we want to add as a CC member has generated Cold keys and has given us the Key hash `89181f26b47c3d3b6b127df163b15b74b45bba7c3b7a1d185c05c2de`, we 
can create the proposal with: 
...
#### Update committe to *remove* an existing cc member:

Let's say we want to remove the CC member with key hash 89181f26b47c3d3b6b127df163b15b74b45bba7c3b7a1d185c05c2de
```
cardano-cli conway governance action create-new-committee \
  --testnet \
  --governance-action-deposit 0 \
...
#### Update committe to change the *quorum*:
```
cardano-cli conway governance action create-new-committee \
  --testnet \
  --governance-action-deposit 0 \
...
```
cardano-cli conway governance query gov-state --testnet-magic 42 | jq .ratify.prevGovActionIds.pgaConstitution
{
  "govActionIx": 0,
  "txId": "00caeb6c2db4575acc43be3e8f87b881dccb86283daf16aa2707275cbe7f3451"
}
...
```
cardano-cli conway governance query gov-state --testnet-magic 42 | jq .ratify.prevGovActionIds.pgaCommittee
{
  "govActionIx": 0,
  "txId": "93afe9f59ffdf7e9af98d582ebef5143691941a5236f3ec8464e601c9395bf66"
}
...
```
* Note that we do not need to provide any previous governance action ID on treasury withdrawals.
...
#### Creating the info governance Action
```
...
## Submit the `*.action` action file in a transaction:
...
## Find the governance action ID of your proposal

You may want to find your governance action ID to share it with others on Discord1 and seek their support. The transaction ID and index of the transaction that submitted the 
proposal serve as the action ID.  An efective way to find your governance action ID is querying the governance state and filter by (the prosposer) stake key hash. 
...
```
Use jq to filter `gov-state` output by stake key hash. The output contains all the relevant information about your governance actions, including `actionId`:

```
cardano-cli conway governance query gov-state --testnet-magic 42 \
