...
All governance actions require to be accompanied by an anchor `(url,hash)` with the supporting rationale for the proposed change, this requirement is even more important for 
'info' governance actions. This argument is rooted in the inherent nature of 'info' actions, which lack on-chain consequences and essentially function like polls. In this context, 
the importance of an anchor becomes clear because the content it houses is exactly what governance actors would be voting on. 
