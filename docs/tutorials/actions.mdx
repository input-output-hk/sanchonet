...
Assume you want to add three CC members, they have have generated cold keys and has provided their key hashes
...
```
* Note: If there is a **previously enacted** governance action to update the committe, the proposal must also include `--governance-action-tx-id` and `--governance-action-index`
