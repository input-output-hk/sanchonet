...
To prevent unintended conflicts between governance actions of the same type, it is essential that some governance action includes the governance action ID of
...
The document at the URL can be of a free form. It's important that it should communicate to ada holders the *what* and the *why* of the proposal. This tutorial mostly uses 'https://github.com/Ryun1/metadata/blob/main/cip108/treasury-withdrawal.jsonld' as an example, see [here](https://github.com/Ryun1/CIPs/blob/governance-metadata-actions/CIP-0108/test-vector.md) for more info.
...
cardano-cli conway governance action update-committee \
  ...
  --anchor-url https://github.com/Ryun1/metadata/blob/main/cip108/treasury-withdrawal.jsonld \
...
```
* Note: If there is a **previously enacted** governance action to update the protocol parameters, the proposal must also include `--governance-action-tx-id` and `--governance-action-index`.
