...
Find the last enacted governance action of this type, If the query returns `null` it means the current consitution (if it exists) is not enacted in a governance action, 
but instead supplied on the Conway genesis file.
...
cardano-cli conway governance action create-constitution \
  ...
  --out-file 
```
When there a previously enacted constitution we need to reference the previous governance action id (TXID and INDEX):
...
```bash
utxo_value=$(ccardano-cli conway query utxo --address "$(cat payment.addr)" --output-json | jq '.[keys[0]].value.lovelace')
```
