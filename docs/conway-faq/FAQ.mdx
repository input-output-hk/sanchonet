...
⚠ Because the old `stake_registration` and `stake_deregistration` are to be depreciated in the era following Conway ⚠ 

The new version allows for leger to check deposit amounts and return instantly, matching the other new deposit containing certificates.

### 3. Why do new certificates have a `coin` field?

This has been done so deposit amounts can be checked on the Ledger level. This is also more useful under Voltaire since protocol params are likely to change more frequently than past eras.
...
| Deleg Certificates            |                                             |                             |                                               |
| reg_cert                     | (7, stake_credential, coin)                 | None                        | None                                          |
| keyDeposit                   |                                             |                             | Refund keyDeposit of amount coin             |
...
| stake_reg_deleg_cert         | (11, stake_credential, pool_keyhash, coin)  | stake_credential            | keyDeposit                                    |
| vote_reg_deleg_cert          | (12, stake_credential, drep, coin)          | stake_credential            | keyDeposit                                    |
| stake_vote_reg_deleg_cert    | (13, stake_credential, pool_keyhash, drep, coin) | stake_credential      | keyDeposit                                    |
...
| voting_procedure (not a certificate) |                                       | drep_credential<br/>OR committee_hot_credential<br/>OR stake_pool_keyHash | None       |
| proposal_procedure (not a certificate) |                                   | coin                        | Deposit is returned at the epoch boundary to the `reward_account` that is specified in the proposal. | Treasury Value |
| coin                         | None                                        | None                        | None                                          |
| Treasury Donation            | positive_coin                               | payment_credential          | None                                          |
...
While if we created another source for scripts this would be more work. I can't really think of what would we gain, if we did decide to go that route that is.

### 6. For the proposal policy in regards to a new CC constitution action: How will provided a script affect a script integrity hash from tx body? Will I need take into account cost model of provided script?
